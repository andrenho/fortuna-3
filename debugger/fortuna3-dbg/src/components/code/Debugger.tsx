    ...
    return (
        <div style={{display: "flex", flexDirection: "column", gap: "8px"}}>
            <div style={{display: "flex", justifyContent: "flex-end", alignItems: "baseline", gap: "4px"}}>
                ...
                <select name="projects" style={{fontSize: "18px"}} value={store.selectedProject} onChange={e => store.setSelectedProject(e.target.value)}>
                    { Object.keys(store.debuggingInfo.projects).map(p => <option value={p}>{p}</option>) }
                </select>
            </div>
